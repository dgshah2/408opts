...
namespace op
{
  ...
  __global__ void const_mem_weights(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
  {
    ...
    #define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
       ...
       if((bx < B) && (by < M) && (h < H_out) && (w < W_out)) {
          ...
          for(int c = 0; c < C; c++)    //For all input feature maps (C)
            for(int p = 0; p < K; p++)  //Go through whole KxK filter
              for(int q = 0; q < K; q++)
                y4d(bx, by, h, w) += x4d(bx, c, h+p, w+q) * k4d(by, c, p, q);
        }
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    if(B == 100) { //TODO: Check
      ...
      // Call the kernel
      const_mem_weights<<<gridDim, blockDim, 0>>>(y.dptr_,x.dptr_,w.dptr_, B,M,C,H,W,K);
    }
    else {

      //Optimization B: Shared memory convolution
      int X_tile_width = TILE_WIDTH + K-1;
